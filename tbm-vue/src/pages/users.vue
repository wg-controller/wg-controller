...
async function ResetAttempts(account: UserAccount) {
  ...
  if (result.valid) {
    try {
      if (userDialogEditMode.value) {
        await PATCH_AccountPassword(userBuffer.value!.email, userPassword.value);
      } else {
  ...
  <v-container fluid max-width="1300">
    ...
    >
      <template #[`item.lastActiveUnixMillis`]="{ item }">
        <span v-if="item.lastActiveUnixMillis > 0">{{
          new Date(item.lastActiveUnixMillis).toLocaleString()
        }}</span>
        <span v-else>never</span>
      </template>

      <template #[`item.suspended`]="{ item }">
        <v-chip v-if="item.failedAttempts >= 5" color="red" size="x-small">SUSPENDED</v-chip>
      </template>
      ...
      <template #[`item.actions`]="{ item }">
        <v-menu open-on-click origin="top" width="150">
          ...
          <v-list density="compact">
            <v-list-item class="d-flex flex-row" @click="EditUserDialog(item)">
              <v-list-item-title>Edit</v-list-item-title>
            </v-list-item>
            <v-list-item class="d-flex flex-row" base-color="red" @click="ResetAttempts(item)">
              <v-list-item-title>Reset Attempts</v-list-item-title>
            </v-list-item>
            <v-list-item class="d-flex flex-row" base-color="red" @click="RemoveUser(item.email)">
    ...
    <v-dialog v-model="userDialog" width="460">
      <v-card>
        <v-form ref="userForm" @submit.prevent="ApplyUserDialog">
          <v-card-title v-if="userDialogEditMode" class="text-h6 ma-3"> Edit User </v-card-title>

          <v-card-title v-else class="text-h6 ma-3"> New User </v-card-title>
          ...
          <v-card-actions class="mb-3 mr-5">
            ...
            <v-btn color="secondary" variant="outlined" @click="userDialog = false"> Cancel </v-btn>

            <v-btn v-if="!userDialogEditMode" color="secondary" type="submit" variant="flat">
              Create
            </v-btn>
            <v-btn v-if="userDialogEditMode" color="secondary" type="submit" variant="flat">
              Apply
